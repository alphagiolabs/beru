...
export default function DragOverlay() {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center pointer-events-none"
      ...
      <div className="flex flex-col items-center gap-3 px-8 py-6 rounded-xl" style={{ background: "rgba(0,0,0,0.85)" }}>
        <svg className="w-10 h-10" fill="none" stroke="var(--accent)" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
            d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
        </svg>
        <span className="text-sm font-semibold" style={{ color: "var(--accent)" }}>{t("drop.overlay")}</span>
        {preview && preview.total > 0 && (
          <span className="text-[10px] font-mono" style={{ color: "var(--text-dim)" }}>
            {preview.videos > 0
              ? t("drop.overlayDetected", { count: preview.videos })
              : "—"}
